...
export function parseStudyAttempts(raw: string | null): StudyAttempt[] {
  try {
    ...
    return items.filter((item): item is StudyAttempt => {
      if (!item || typeof item !== 'object') return false;
      const a = item as Record<string, unknown>;
      return ['lessonId', 'questionId', 'answer', 'at'].every(
        (key) => typeof a[key] === 'string' && (a[key] as string).length > 0 && (a[key] as string).length <= 2000,
      ) && typeof a.correct === 'boolean' && typeof a.assisted === 'boolean'
...
export function recordStudyAttempt(
  attempts: StudyAttempt[], wrongQuestions: WrongQuestion[], attempt: StudyAttempt, wrong?: WrongQuestion,
) {
  return {
    attempts: [...attempts, attempt].slice(-5000),
    // Preserve the first failure; later evidence lives in the attempt history.
    wrongQuestions: !attempt.correct && wrong && !wrongQuestions.some((item) => item.questionId === wrong.questionId)
      ? [wrong, ...wrongQuestions] : wrongQuestions,
  };
}
...
export function getStudyStatus(attempts: StudyAttempt[], lessonId: string) {
  ...
  const history = attempts.filter((a) => a.lessonId === lessonId)
    .sort((a, b) => Date.parse(a.at) - Date.parse(b.at));
  for (const a of history) {
    if (!a.correct || a.assisted) {
      label = '需要提示'; independent.clear(); reviews = 0;
      dueOn = afterDays(studyDate(new Date(a.at)), 2);
      continue;
    }
    if (a.mode === 'practice') {
      independent.add(a.questionId);
      if (independent.size < 2) { label = '继续练习'; continue; }
      if (label !== '独立完成' && label !== '隔时迁移通过') {
        label = '独立完成'; dueOn = afterDays(studyDate(new Date(a.at)), 2);
      }
    } else if (independent.size >= 2 && dueOn && studyDate(new Date(a.at)) >= dueOn && !independent.has(a.questionId)) {
      label = '隔时迁移通过'; reviews += 1;
