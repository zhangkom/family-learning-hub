...
export function ChildDashboard({ plan }: { plan: ChildPlan }) {
  ...
  const progress = useMemo(
    () => Math.round((checked.length / plan.tasks.length) * 100),
    [checked.length, plan.tasks.length],
  );

  return (
    <main
      className={`plan-page plan-page--${plan.accent} min-h-screen bg-background text-foreground`}
    >
      ...
      <div className="mx-auto max-w-7xl px-5 py-7 lg:px-8 lg:py-9">
        <section className="plan-banner grid gap-6 rounded-[1.75rem] p-6 md:grid-cols-[1fr_auto] md:items-end lg:p-8">
          ...
          <div className="min-w-60 rounded-2xl bg-white/75 p-4 backdrop-blur-sm">
            ...
            {plan.workbench.secondary && (
              <Link
                href={plan.workbench.secondary.href}
                className="mt-2 inline-flex min-h-10 w-full items-center justify-center gap-2 rounded-xl border border-foreground/10 bg-white/60 px-4 text-sm font-bold text-foreground transition hover:bg-white"
              >
                {plan.workbench.secondary.label}{' '}
        ...
        <Tabs defaultValue="week" className="mt-7">
          <TabsList className="h-auto w-full justify-start gap-1 overflow-x-auto rounded-2xl border border-border bg-card p-1.5 sm:w-fit">
            <TabsTrigger value="week" className="min-h-9 px-3">
              本周执行
            </TabsTrigger>
            <TabsTrigger value="subjects" className="min-h-9 px-3">
              学科地图
            </TabsTrigger>
            <TabsTrigger value="roadmap" className="min-h-9 px-3">
              倒推路线
            </TabsTrigger>
            <TabsTrigger value="rules" className="min-h-9 px-3">
              考试规则
            </TabsTrigger>
          </TabsList>
              ...
              <section className="rounded-3xl border border-border bg-card p-5 sm:p-6">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  ...
                  <span className="rounded-full bg-muted px-3 py-1.5 text-xs font-semibold">
                    共 {plan.tasks.reduce((sum, item) => sum + item.minutes, 0)}{' '}
                    分钟
                  </span>
                </div>
                ...
                <div className="mt-6 space-y-3">
                  {plan.tasks.map((task) => {
                    ...
                    return (
                      <label
                        key={task.id}
                        className={`group flex cursor-pointer items-start gap-3 rounded-2xl border p-4 transition ${done ? 'border-primary/20 bg-secondary/55' : 'border-border bg-background hover:border-primary/25'}`}
                      >
                        <Checkbox
                          checked={done}
                        ...
                        <span className="shrink-0 rounded-full bg-card px-2 py-1 text-xs text-muted-foreground">
                          {task.minutes} 分钟
                        </span>
                      </label>
                    );
              ...
              <aside className="space-y-5">
                  ...
                  <ol className="mt-4 space-y-3 text-sm">
                    {[
                      ...
                      <li key={item} className="flex gap-3">
                        <span className="grid size-6 shrink-0 place-items-center rounded-full bg-white/10 text-xs text-sun">
                          {index + 1}
                        </span>
                ...
                <div className="rounded-3xl border border-border bg-card p-5">
                  <p className="flex items-center gap-2 font-bold">
                    <Lightbulb className="size-5 text-primary" /> 题库怎么用
                  </p>
                  <p className="mt-3 text-sm leading-6 text-muted-foreground">
                    每周按错因生成 2–3 组“小题包”，每组 5–8
                    题。正确率连续两次达到 85% 再升级，不以刷题数量作为目标。
                  </p>
                </div>
              </aside>
            </div>
              ...
              {plan.subjects.map((subject) => (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span
                      className={`rounded-full px-2.5 py-1 text-xs font-semibold ${statusStyles[subject.status]}`}
                    >
                      {subject.status}
                    </span>
                  </div>
            ...
            <section className="rounded-3xl border border-border bg-card p-5 sm:p-7">
              ...
              <div className="relative space-y-1 before:absolute before:bottom-5 before:left-[15px] before:top-5 before:w-px before:bg-border">
                {plan.roadmap.map((step, index) => (
                  <div
                    key={step.term}
                    className="relative grid grid-cols-[32px_1fr] gap-4 py-3"
                  >
                    <span className="z-10 grid size-8 place-items-center rounded-full border-4 border-card bg-primary text-xs font-bold text-primary-foreground">
                      {index + 1}
                    </span>
            ...
            <div className="grid gap-5 lg:grid-cols-[1fr_.65fr]">
              <section className="rounded-3xl border border-border bg-card p-5 sm:p-7">
                <p className="flex items-center gap-2 text-sm font-semibold text-primary">
                  <School className="size-4" /> 官方口径摘要
                </p>
                ...
                <div className="mt-5 space-y-3">
                  {plan.policyFacts.map((fact) => (
                    <div
                      key={fact}
                      className="flex gap-3 rounded-2xl bg-muted/70 p-4 text-sm leading-6"
                    >
              ...
              <aside className="rounded-3xl border border-border bg-card p-5">
                <p className="flex items-center gap-2 font-bold">
                  <CircleAlert className="size-5 text-primary" /> 使用提醒
                </p>
                <p className="mt-3 text-sm leading-6 text-muted-foreground">
                  政策、招生专业组和投档位次会变化。网页中的数字是倒推坐标，不是录取承诺；每年
                  7–9 月用最新官方数据刷新一次。
                </p>
                <div className="mt-5 space-y-2">
                  {plan.sources.map((source) => (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="flex items-center justify-between rounded-xl border border-border px-3 py-3 text-sm font-semibold hover:border-primary/30 hover:bg-muted/60"
                    >
                      {source.label}
