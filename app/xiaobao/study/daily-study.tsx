...
const lessons: Lesson[] = [
  {
    ...
    explanation:
      '数轴上向右移动表示增加，向左移动表示减少。先写起点，再把移动方向翻译成带符号的数。',
  ...
  },
  {
    ...
    explanation:
      '一般现在时中，主语是 he、she、it 或一个人/物时，实义动词通常加 -s；以 ch 结尾的 watch 加 -es。',
...
export function DailyStudy() {
  ...
  const [syncState, setSyncState] = useState<'loading' | 'synced' | 'local'>(
    'loading',
  );
  const lesson = useMemo(
    () => lessons.find((item) => item.subject === subject)!,
    [subject],
  );
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const cloudQuestionIds = new Set(
          cloudItems.map((item) => item.questionId),
        );
        const pendingItems = localItems.filter(
          (item) => !cloudQuestionIds.has(item.questionId),
        );
  ...
  const checkAnswer = () => {
    ...
    if (!correct) {
      ...
      const next = [
        entry,
        ...wrongQuestions.filter((item) => item.questionId !== question.id),
      ];
          ...
          <div className="flex flex-wrap items-start justify-between gap-5">
            <div>
              ...
              <p className="mt-3 text-sm text-muted-foreground">
                理解 5 分钟 · 针对练习 10 分钟 · 讲给爸爸听 5 分钟
              </p>
            </div>
            <div className="flex rounded-2xl bg-white/75 p-1.5">
              {(['数学', '英语'] as const).map((item) => (
                <button
                  key={item}
                  onClick={() => selectSubject(item)}
                  className={`min-h-11 rounded-xl px-5 text-sm font-bold ${subject === item ? 'bg-ink text-white' : 'text-muted-foreground'}`}
                >
                  {item}
                </button>
              ))}
          ...
          <section className="rounded-3xl border border-border bg-card p-5 sm:p-7">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <span className="flex items-center gap-1.5 text-xs text-muted-foreground">
                <Clock3 className="size-4" /> 约 20 分钟
              </span>
            </div>
            ...
            <div className="mt-5 rounded-2xl bg-muted/70 p-4">
              <p className="flex items-center gap-2 font-bold">
                <BookOpenCheck className="size-5 text-primary" /> 先理解
              </p>
              <p className="mt-2 text-sm leading-6 text-muted-foreground">
                {lesson.explanation}
              </p>
            ...
            <div className="mt-5 border-t border-border pt-5">
              ...
              <div className="mt-4 flex flex-col gap-3 sm:flex-row">
                <input
                  ...
                  placeholder="在这里写答案"
                  className="min-h-12 flex-1 rounded-xl border border-input bg-background px-4 text-base outline-none focus:border-primary focus:ring-2 focus:ring-primary/15"
                />
                <button
                  onClick={checkAnswer}
              ...
              {feedback === 'correct' && (
                <div className="mt-4 rounded-2xl bg-[#e7f5f2] p-4 text-sm text-[#176a62]">
                  <p className="flex items-center gap-2 font-bold">
                    <Check className="size-4" /> 做对了
                  </p>
                  <p className="mt-1">
                    现在请不用看网页，把“为什么”讲给爸爸听。
                  </p>
                </div>
              )}
              {feedback === 'wrong' && (
                <div className="mt-4 rounded-2xl bg-[#fff0e9] p-4 text-sm text-[#8e3a22]">
                  <p className="flex items-center gap-2 font-bold">
                    <RotateCcw className="size-4" /> 已收入错题本
                  </p>
                  <p className="mt-2 leading-6">
                    正确答案：{question.accepted[0]}。{lesson.tip}
                  </p>
                  {!useVariant && (
                    <button
                      onClick={startVariant}
                      className="mt-3 inline-flex items-center gap-1 font-bold underline underline-offset-4"
                    >
                      马上做一道变式 <ChevronRight className="size-4" />
                    </button>
                  )}
                </div>
              )}
          ...
          <aside className="space-y-5">
            ...
            <section className="rounded-3xl border border-border bg-card p-5">
              <div className="flex items-center justify-between">
                <p className="flex items-center gap-2 font-bold">
                  <Sparkles className="size-5 text-primary" /> 当前错题本
                </p>
              ...
              {wrongQuestions.length === 0 ? (
                ...
                <div className="mt-4 space-y-3">
                  {wrongQuestions.slice(0, 3).map((item) => (
                    ...
                    >
                      <p className="font-bold">{item.knowledgePoint}</p>
                      <p className="mt-1 line-clamp-2 text-xs leading-5 text-muted-foreground">
                        {item.prompt}
                      </p>
                      <p className="mt-2 text-xs text-primary">
                        下次复习：{item.reviewDates[0]}
                      </p>
                    </div>
                  ))}
                </div>
            ...
            <section className="rounded-3xl border border-dashed border-border bg-card p-5">
              <p className="flex items-center gap-2 font-bold">
                <Lightbulb className="size-5 text-primary" /> 后续学科
              </p>
              <p className="mt-2 text-sm leading-6 text-muted-foreground">
                语文、物理、化学、生物、历史、道法、地理已预留，按学校进度逐步开启。
              </p>
            </section>
          </aside>
        </div>
