...
const paths = ['/', '/xiaobao', '/dabao', '/xiaobao/study', '/xiaobao/practice', '/dabao/practice', '/xiaobao/wrong-book', '/dabao/review'];
let exerciseCount = 0;
for (const path of paths) {
  ...
  if (path.endsWith('/practice')) {
    const links = [...new Set([...html.matchAll(/href="(\/family-learning\/(?:xiaobao|dabao)\/practice\/[^"?#]+)"/g)].map(m => m[1]))];
  ...
  const assets = [...new Set([...html.matchAll(/(?:src|href)="(\/family-learning\/_next\/static\/[^"?#]+)"/g)].map(m => m[1]))];
...
for (const path of ['/api/scans', '/api/wrong-questions?child=xiaobao', '/api/scans/test/file']) {
